...
def main():
    ...
    solar_cap = (
        cap[cap["ptype_norm"].str.contains("solar")]
        .groupby("year")["MW"]
        .sum()
    )
    wind_cap = (
        cap[cap["ptype_norm"].str.contains("wind")]
        .groupby("year")["MW"]
        .sum()
    )
    ...
    # Map annual capacities to hourly
    ...
    T_K = T_C + 273.15
    p_Pa = df["surface_pressure (hPa)"] * 100
    RH = (df["relative_humidity_2m (%)"] / 100).clip(0, 1)
    ...
    df["pv_proxy"] = (
        df["shortwave_radiation (W/m²)"].clip(lower=0)
    ...
    # Make holidays UTC-aware -> normalize -> drop tz to match naive index
    hol["date"] = (
        hol["date"]
        .dt.tz_convert("UTC")
        .dt.normalize()
    ...
    print(
        f"Saved engineered CSVs to {cfg['engineered_csv']} "
        f"and {cfg['engineered_full_csv']}"
    )
