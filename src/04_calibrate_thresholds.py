...
def make_supervised(df, horizon=1, lags=(1,2,3,6,12,24,48,72), rolls=(3,6,12,24,48)):
    ...
    for c in core:
        ...
        for R in rolls:
            X[f"{c}_rmean{R}"] = df[c].rolling(R, min_periods=max(2, int(0.6*R))).mean()
    Y = pd.DataFrame({
...
thresholds = {"meta": {"method": METHOD, "alpha": ALPHA, "bucket_by": BUCKET_BY},
              "targets": {}}
...
def rolling_mad_threshold(series, alpha):
    # median +/- k * MAD; approximate k for coverage ~ alpha
    k = 2.0 * np.arctanh(alpha - 1e-6)
...
# ----------------- Residual plot (standardized per target) -----------------
os.makedirs("reports/figures", exist_ok=True)
val = df.loc[SPLIT["val_start"]:SPLIT["val_end"]].copy()

plt.figure(figsize=(10, 6))
for name in target_names:
    r = val_df[f"{name}_resid"].dropna().values
    if r.size == 0 or not np.isfinite(r).any():
        continue
    r = (r - np.nanmean(r)) / (np.nanstd(r) + 1e-9)  # z-score per target
    hist, bins = np.histogram(r[np.isfinite(r)], bins=200, density=True)
    centers = 0.5 * (bins[1:] + bins[:-1])
    plt.plot(centers, hist, label=str(name))
plt.title("Validation residual distribution (z-scored per target)")
